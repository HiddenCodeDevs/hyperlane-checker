...
URL_TEMPLATE = "https://claim.hyperlane.foundation/api/check-eligibility?address={}"
...
def process_wallet(wallet: str, scraper: cloudscraper.CloudScraper) -> None:
    ...
    try:
        ...
        if response.status_code == 200:
            try:
                data = response.json()
                points = data.get("response", {}).get("eligibilities", [])

                if points and len(points)>0:
                    try:
                        value = float(points[0]['amount'])
                        print(f"[*]{wallet} is eligible (value: {value:.2f})")
